...
class Solution {
    public int[] findOrder(int numCourses, int[][] prerequisites) {
        ...
        // map courseA -> courses that depend on courseA
        for(int[] req : prerequisites) {
            map.get(req[1]).add(req[0]);
        }
        int[] visited = new int[numCourses];
        for(int i =0; i < numCourses; i++) {
            if(!dfs(map, visited, stack, i)) {
                // 出现异常情况, 无法完成课程表
                return new int[0];
            }
        }
      ...
      public boolean dfs(List<List<Integer>> map, int[] visited, Deque<Integer> stack, int course) {
          if(visited[course] == 1) {
              return false;
          ...
          for(Integer nextCourse : map.get(course)) {
              if(visited[nextCourse] ==0) {
                  if(!dfs(map, visited, stack, nextCourse)) {
                      return false;
                  }
